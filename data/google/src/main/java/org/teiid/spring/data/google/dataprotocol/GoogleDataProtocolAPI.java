 ...
 * The most important method is executeQuery.
 *
 * @author fnguyen
 *
 */
public class GoogleDataProtocolAPI {
    ...
    public void setCredentialUtil(GoogleCredentialUtil util) {
        this.credentialUtil = util;
    }

    /**
     * Most important method that will issue query [1] to specific worksheet. The
     * columns in the query should be identified by their real alphabetic name (A,
     * B, C...).
     *
     * There is one important restriction to query. It should not contain offset and
     * limit clauses. To achieve functionality of offset and limit please use
     * corresponding parameters in this method.
     *
     *
     ...
     *         Google for data
     */
    public RowsResult executeQuery(SpreadsheetInfo info, String worksheetTitle, String query, int batchSize,
    ...
    /**
     * Logic to query portion of data from Google Visualization Data Protocol. We do
     * not use any special library just simple Http request. Google sends response
     * back in CSV that we parse afterwards.
     *
     * @author fnguyen
     *
     */
    public class DataProtocolQueryStrategy implements PartialResultExecutor {
        ...
        public List<SheetRow> getResultsBatch(int startIndex, int amount) {
            ...
            HttpGet get = new HttpGet("https://spreadsheets.google.com/tq?key=" + spreadsheetKey + "&sheet=" + worksheet //$NON-NLS-1$ //$NON-NLS-2$
                    + "&tqx=responseHandler:x;out:json&tq=" + boundariedQuery); //$NON-NLS-1$
            get.setHeader("Authorization", credentialUtil.getAuthHeader()); //$NON-NLS-1$
            ...
            try {
                ...
                try {
                    ...
                    // relogin
                    credentialUtil.refreshToken();
                    get.setHeader("Authorization", credentialUtil.getAuthHeader()); //$NON-NLS-1$
                    return executeAndParse(client, get);
                }
            } catch (IOException e) {
                throw new SpreadsheetOperationException(
                        "Error retrieving batch from Gogole Visualization Data protocol", e);
            }
        }
        ...
        private List<SheetRow> executeAndParse(HttpClient client, HttpGet get) throws IOException {
            ...
            if (response.getStatusLine().getStatusCode() == 200) {
                ...
                // 500 from server may not be a actual error. It can mean that offset is higher
                // then actual result size. Can be solved by calling "count" first but
                // performance penalty
                return new ArrayList<SheetRow>();
            } else {
                throw new SpreadsheetOperationException("Error when getting batch "
                        + response.getStatusLine().getStatusCode() + ":" + response.getStatusLine().getReasonPhrase());
            }
        }

        /**
         * Adds limit, offset to the query. This is slightly more complicated becuase
         * limit/offset must appear before certain clauses (label, format, options)
         *
         * @param amount
         * @param offset
         * @return
        ...
        private String getQueryWithBoundaries(int amount, int offset) throws UnsupportedEncodingException {
            ...
            return urlEncodedQuery.substring(0, indexToPut).toString()
                    + URLEncoder.encode(" limit " + amount + " offset " + offset + " ", ENCODING).toString() //$NON-NLS-1$ //$NON-NLS-2$ //$NON-NLS-3$
